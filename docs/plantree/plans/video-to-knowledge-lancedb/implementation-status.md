...
### 2026-09-06 日志证据保留与解析器限频恢复

- 范围：承接 D9；24 房间保持停止，不新增服务、不改 Cookie/媒体/数据库，不恢复监听。运行态读取仍为 `80ab0ec`、active=0。
- 启动日志 RED 已复现：真实 `log.New` 子进程连续启动两次，普通模式删除昨日证据和当日日志；Launcher 对照不丢失。删除启动全量清理，仅保留 RotateDays 清理与追加写；回归 GREEN。
- 历史采用新增 `--evidence-dir`：dry-run 独占创建私有目录，保存完整原日志与计划；应用仍重读归档日志、核验 SHA/生命周期/任务/媒体。原日志缺失不能创建归档；已存在不覆盖，日志复制期间变化拒绝。不从已有摘要重建 961/972 证据，两项仍未恢复。
- 500 处置边界：确认常驻解析器返回全端点禁用、新进程未换 Cookie 可用；最初触发原因未知。上游 `3.1.2-bgo.2` 及当前 master 均有到期解禁，但并发在途失败继续叠加指数退避，不能写成完全无恢复机制。本轮不热改第三方 bundle；在现有 Go 进程管理入口增加兜底恢复，不宣称修复上游初始失败。
- 上游源码确定性重放：`loadBalancer.ts` SHA256 `516d0ec36de3b0e06fcaf0cd115d208863e81ba8d3b6b3582a3bcbc5f92808e3`；固定时钟/选端点顺序，24 路并发失败后 failureCount 为 `26/26/18/26`，最短 retry 为 `1313.68` 分钟。将接口改为成功并推进三分钟，旧实例仍全禁用且外发调用=0，新实例成功。证明并发退避放大机制，非生产实际禁用时长、非最初网络失败来源。
- 恢复契约：只有 live-info 的精确已知 500 分类持续三分钟才停止当前解析器子进程，等待退出再启动；五分钟最多一次。有效 live-info 清除窗口；代际标识拒绝旧响应，Cleanup 阻止重启。不得调用全局进程清理，不影响 FFmpeg/worker/app；未知 500/网络失败不触发恢复、不当离线。HTTP 统一 30 秒超时、1 MiB 响应上限，只显示固定错误分类，不透传上游敏感正文；缺少有效 living 字段拒绝离线判定。
- 验证：真实解析子进程先 RED（持续禁用不恢复）再 GREEN；三分钟边界、五分钟限频、24 并发仅一次恢复、旧响应、成功清窗、停止与启动失败通过。review 发现继承管道阻塞，真实 RED 后以 WaitDelay=2s 修复，三包 race 全绿。Node20.10.0 的 `make build-web dev lint test` 与 Python 40 项通过；首轮默认 Node23 engine 不兼容、全量测试本机 EADDRNOTAVAIL，换仓库 Node版本及定向/全量复跑通过，未改网络。待中文 PR/CI/发布和最小部署；生产长期恢复及真实录制/投影仪尚未验收。

### 2026-09-05 烧录发布契约根因修复：方案澄清

- 状态：两批均已发布部署。第一批 [PR #51](https://github.com/Neymar022/bililive-go/pull/51) 合并 `5f7194b8eac69067814791a8db49f522c462cdc0`，09-06 09:24 +08 部署；第二批 [PR #52](https://github.com/Neymar022/bililive-go/pull/52) 合并 `80ab0ec48c1ae54882c5eb3c0b939d7ea39c2491`，10:14 左右以原 Compose/latest 最小 app 部署，10:21 运行 SHA、digest、worker、原媒体及真实 NAS Web 后验通过。天津/熊小电 NFO 和必要四行 PG 修复已完成；历史采用/retry、真实录制/投影仪与防复发验收未完成。两批必要 CI 全绿，不重复全量 gate；既存未跟踪 `scripts/apply-chronological-renumber.py` 和两个 `__pycache__` 保留。
- D9（本轮明确授权）：用户将部署/修复置于录制之前，要求立即停止所有录制并删除临时录制文件/缓存。10:07 已停止全部 24 房间监听并 PUT config 持久化；不自动恢复监听。清理限定本次 session977/978、task1536/1537 的两个未发布 capture MP4，共 `464591065` bytes；先保存任务/文件清单、取消任务，停 app 后 CAS 设置 `can_retry=0` 和删除原因，校验无其他任务/公开 JSON 引用、inode/size/mtime/nlink 后删除，移除空 capture 和四个空父目录。保留取消审计记录；不删除既存媒体、其他历史输入或模型缓存。原 task1535 及其 `not_before=09-07 02:00 +08` 完整保持。
- 10:21 生产后验：`/api/info.git_hash=80ab0ec48c1ae54882c5eb3c0b939d7ea39c2491`，container image 为 `neymar022/bililive-go-app:latest`，image ID `sha256:bb48d032b7c94aa62465f84ebb2abbd40f5281bbba08c48a97ae08d9ab462acd`，digest 与授权镜像 `sha256:4c7e74a7f001867beb319e37b35bac7277e9af9531b6da932fc0f4dd2cfc441e` 一致；worker ID/image/StartedAt 未变、其 network namespace `/healthz`=200。房间 24、listening/recording/active/ffmpeg=0；旧房间模式均 continuous。真实 NAS Web 添加窗口默认 once，点击连续/单次标签切换通过、无横向溢出；未提交新增房间。24 行均“已停止”，可见 500=0；内置 btools 单房间只读探针 200，不作为持久防复发证明。
- 10:20 历史元数据已写入并后验：天津 `60` 集、熊小电 `4` 集，各自公开 episode 连续且按录制时间排序；天津 60 NFO 补独立 identity/末集号、熊小电 1 NFO 改号。UGREEN 四行 CAS/零冲突门禁后 COMMIT；两张表各 46 范围行与原快照逐字段对比，只有指定两集的 episode 字段改变，其他关系/进度/收藏字段未变。1262 个既存媒体 inode/size/mtime 全部不变。证据、配置/SQLite 增量备份和清理清单位于同次备份的 `priority-stop/`，NFO 原件位于 `ordinals-tianjin/`、`ordinals-xiong/`；整个本轮备份约 67 MB，无 MP4 副本，仍遵循最终验收后七天、删除另行确认。
- 历史采用新增阻塞：`bililive-go-2026-09-04.log`、`09-05.log` 已不在 NAS 根目录树，本轮 `adopt-recording-session.py` 刷新因原日志缺失拒绝；task1518/1532 保持 failed，未采用/retry。已定位 `src/log/log.go:55-64` 在非 Launcher 启动且 SaveLastLog=true 时删除全部 `bililive-go-*.log`，实际配置匹配；首批部署前未将采用依赖的日志纳入备份，属于部署检查遗漏。已保存的 dry-run 含 SHA/行号而非原日志全文，不能冒充完整原证据重建日志或绕过校验。后续先补日志保留和采用证据归档机制，恢复原证据或重新形成可审计方案后再续跑。500 的持久恢复机制同样未改，不把新进程暂时 200 写为永久修复。
...
- 运行态依据：诊断时生产与 master 同为 `0ce2dd5`，证明旧修复已部署但未覆盖两条入口；09-06 第一批部署后 `/api/info.git_hash=5f7194b8eac69067814791a8db49f522c462cdc0`。任务错误发生在 worker 调用前，worker `/healthz` 为 200，存在诊断当日 `actual_burn_provider=remote-mac` completed 证据。易变运行数值以执行前新鲜 API 为准。
...
- 第一批源码已发布部署：普通/聚合统一 NFO writer；有效 MP4/MKV 排序；SQLite 场次/producer/完整输入集合；三分钟离线确认；确定性库外处理；完整场次公开与知识开关解耦；`retry?mode=resume` 保留检查点/计划并比较交换；历史工具缺失 uniqueid 恢复、独立小 NFO 备份、并发变更拒绝自动回滚。历史生产维护结果不由源码部署代替。
...
- 最新生产：09-06 用户语音明确批准临时停止王老师失败重试并继续部署；09:20 停该房间后 active=0，running=0、update=idle、ffmpeg/近五分钟媒体写入=0，pending1535 完全保留。第一批部署后解析恢复；09:24 装修观察员孙壹萌被识别为真实开播，已产生库外 capture，09:29 active=1、recording=true、ffmpeg 存在、running/pending=0/2。这是新的真实录制门禁，不是权限或空转重试；不打断该房间。王老师原监听已恢复，status=false、recording=false。sudo 经关闭回显的 SSH PTY 受控 stdin 验证，密码未写入脚本/环境/文件；本轮 SSH 已退出。历史 apply/adopt/retry 和第二批部署均未执行。
- 第一批生产证据：备份 `/volume2/docker/bililive-go/backups/publication-once-20260906-092009/` 约 18.1 MB，含原配置/Compose、SQLite 一致性备份、容器/镜像信息、受影响 PG 行、两组新鲜 dry-run、1262 个既存媒体的 inode/size/mtime 清单；不复制 MP4。原 1262 个媒体缺失/变更=0，task1535 当前完整 API 与备份相同。app restart=0；worker ID/image/StartedAt 均未变、healthz=200。回滚 tag `neymar022/bililive-go-app:rollback-20260906-092009`。活动第一批使用原 Compose 加同备份内 `phase1.override.json`；原 Compose 仍为 latest，第二批部署时须仅用原 Compose、恢复 latest 实际运行，不能重复部署第一批。备份最终验收后保留七天，删前另行确认。
- 新增 500 诊断：23/24 房间报错均为抖音；生产 btools `3.1.2-bgo.2` 的 `/bgo/live-info` 连续两次 500，body 为全部 API 端点不可用；channel-info 两次 200，stream-info 报非直播。运行 bundle 的全局 APILoadBalancer 封锁所有端点；同容器、同网络、同代码的独立只读进程，不提供新 Cookie，web/webHTML/balance 三路径均成功返回 offline。该 bgo 路由未传用户 Cookie，不能把本例归因于用户 Cookie 过期。app 最小部署重启内置 btools 后错误清零；09:29 新开真实 NAS Web 页 24 行、告警图标=0、可见/API 500=0，截图已通过工具目视核验。常驻熔断状态未恢复为直接故障，最初触发失败的原因尚未证明；未修改 Cookie/代理/btools 代码，不宣称永久防复发。后续方向为上限退避/半开恢复与错误正文脱敏透传，不用反复换 Cookie 或盲目重启替代根因修复。
- 部署前挂载快照：app ID `0cf9eec54674...`、worker ID `d2cec0d0b219...`，同属 `bililive-go` 项目，整根同路径 bind 均 RW；该旧 app 已由第一批替换，不再代表运行态。当前 app 完整 inspect 在本次备份 `containers-phase1.json`；worker 未变，restart=1、StartedAt `2026-08-31T18:20:02Z`。写入门禁必须重新 inspect，不复用旧容器 ID。
- 09:51 +08 有界续跑：生产仍为第一批、active=1，孙壹萌 recording=true 且 ffmpeg 存在；running/pending=0/2、update=idle、房间错误=0。task1536/session977 已持久化 producer 和 1 个 task，ready=false、end_reason 未设置；当前 1 个输入实际存在且位于库外工作区，未等计划时间强跑。四行 PG 维护准备完成：同次备份中的 `four-rows-dry-run.sql` 以 READ ONLY + EXPLAIN 在真实 PostgreSQL 执行通过，四行 exact_rows 均为 1、target_conflicts 均为 0；`four-rows-apply.sql`/`four-rows-rollback.sql` 具完整行 CAS、目标冲突断言、4 行计数断言、事务和锁超时。仅生成并只读检查，未执行 UPDATE/回滚；应用前仍需新鲜门禁与受影响文件/DB fixed point。生成器为忽略的 `test-output/prepare-ugreen-four-rows.py`，NAS 副本在已有临时工具目录；不新增服务/PR/文档。SSH 已退出，heartbeat 配置仍 ACTIVE；恢复后的新定时触发尚未取得证据，本轮为用户消息触发。
- 09:54 heartbeat 已实际唤醒，09:56-09:58 +08 只读发现 500 复发：22 个监听中的抖音房间均有 `last_error=请求失败: 500 Internal Server Error`；未监听的建筑师和哔哩哔哩房间无错误。当前 app `ab7c6fb2f83e...`、StartedAt `2026-09-06T01:23:40.667410094Z`、restart=0、SHA 仍为第一批。在其网络命名空间直接请求内置 `/bgo/live-info` 得到 HTTP 500、`所有 API 端点都不可用，请稍后重试`，与旧故障响应一致；本轮未重新执行独立 Node 对照，最初触发原因仍未知。孙壹萌仍 recording=true、active=1、ffmpeg=1，running/pending=0/2、update=idle；API 的 status 可能因请求失败滞留，不将其当作新鲜开播/离线证据，不停止录制或重启解析服务。未维护写入、retry 或部署第二批；所有 SSH 已退出。恢复只是暂时的，后续监控不得继续报“房间错误=0”或永久修复。
- 08:14 +08 root 只读 fixed point：天津 60 MP4 / 48,164,582,788 bytes，60 NFO 待补独立 identity，其中末集长公开号改 60，fingerprint `37572f6628848da751ccfe570727f1463811aeafe39b4017f52f1d49a61e4489`；熊小电 4 MP4 / 7,807,358,890 bytes，仅 1 NFO 长公开号改 4，fingerprint 仍为 `760ec65d50ce1ce0c73f6cf7c1ae09dc237e9c8615fa4ec62631fac1d19d17ff`。普通 SSH 读取 root 私有 sidecar 的 PermissionError 不代表生产 root 容器不可读；未放宽文件权限。
- F1/F2 样本采用：task1518/session961 3 MP4 / 1,548,734,590 bytes，fingerprint `be1fc31ed47d2aabd432a9cf025a3922e180a1fcadbdc65c0995200ca9fdffa7`，起点 09-04 18:01:35，日志 126642/127716/128351/128352；新增 F2 代表 task1532/session972 1 MP4 / 2,235,853,923 bytes，fingerprint `c76228ce540ab81e392f29204ef5e6b88d9b963c7c13338b06a1e6d5261eb7ca`，起点 09-05 20:51:47，日志 241887/244092/245119/245120。两者 dry-run GREEN、仍 failed，未采用/重试。task1523/session966 因 session start=22:01:21、初始文件=22:01:22 而 RED；保留排除，不放宽时间证据、不重试。
- UGREEN 只读 DB：root 可通过 `/usr/bin/psql -X -w -U postgres -p 5433 -d video` 本机连接，无新增凭据。天津末集 file_info ID `26660`、category `ca07758a8e6df3d686f4e01dca646470`、episode ID `15946`，长号 `1676482048871984`；熊小电对应 `26674`、`01c94e471f0b069051420bf2d0109e73`、`15962`、长号 `1677961904665216`。两集 season=1、current_play_time=0；episode.file_path=[]，应按 category/season/episode 核对，不能以数组 join 为空认定丢失。写前仍需证明目标 60/4 无冲突、完整旧值与关联稳定，精确备份后仅改两个 file_info.episode_num 和两个 episode.episode；其他关系/进度/收藏不动。
...
- 第二批发布：PR CI `33978855480`、Warmup/build-web `33978855510`、E2E `33978855420` 全绿；Claude review `33978855433` 为已证 GitHub App 未安装的 401 外部失败，未绕过权限或盲目重跑。合并后 CI `33979245237`、E2E `33979245112`、发布 `33979245109`、Warmup `33979245257` 于 01:00 +08 全绿。Docker Hub `latest` 现已前移至 `sha-80ab0ec`，二者 digest 均为 `sha256:4c7e74a7f001867beb319e37b35bac7277e9af9531b6da932fc0f4dd2cfc441e`，amd64 manifest `sha256:d7dfc685aa1008c72d7c3065e3b97d3d7d146a92f318132d44affcbda2528a0b`；第一批 SHA tag 仍保留，不能把旧脚本的 latest 当作第一批版本。
- 下一步：两批已部署、天津/熊小电元数据已修复，不重复部署或重新 apply。当前无录制门禁；保留全部房间停止状态。先处理采用日志证据缺口及解析服务 500 持久恢复，不能绕过历史输入封口核验；旧 task1535 保留计划，不强跑。Mac/投影仪、聚合后下一次发布和真实单次录制生命周期仍待验收，不标整体 complete。
- 有界续跑：heartbeat `ugreen` / `UGREEN 安全窗口续跑`，此前已真实唤醒。10:07 本轮独占生产操作前暂停；不再等待录制自然结束或按旧提示重复部署。当前剩余阻塞不是安全窗口，保持 PAUSED 并更新交接提示；后续恢复应围绕源码/证据问题，不轮询同一 idle 状态，不自动恢复监控。配置不含密码，不伪标 Goal complete。
...
- 用户真实页面把问题固定到不同 seam：序号视图因上一版把超长 episode 标签改成空串而显示成不可判断的灰块；卡片视图的 `HorizontalList` 子组件已发出 `select`，但剧集页父组件未绑定 `handleChangeEpisode`，且通用居中滚动会让首卡停在左侧裁切位置。序号最初按 `MM-DD` 技术方案恢复，用户后续把最终产品目标改为按当前排序显示 ordinal；tooltip 仍保留完整日期标题，底层 MP4/NFO/DB/JSON/manifest 的长 recordedAt identity 不变。卡片整卡点击和首卡完整可见继续独立验收。
...
- 当前源码工作树基于 `origin/master=d21284a12419c8887f197d8a07d99d39dc9b7617`。patcher 显式识别 vendor、已部署 title-v1/date-v2、ordinal-v3 和 full-final；`date-v2 -> ordinal-v3` 只替换 serial seam，卡片点击、光标与滚动保持原状。未知或混合状态 fail closed；JS/Gzip 在写前必须具有相同 bundle state 和解压后内容，备份后才原子写入，失败会继续尝试回滚全部已写资产。
...
- 正式 Standards/Spec 双轴复审均为 `0 findings`，但该结论对应此前的 `MM-DD` 标签规格。生产最小恢复在 `2026-08-19 23:19 +08` 通过 Computer Use 技术验收：`建筑师 linkai` 的 53 个 `MM-DD` 标签与 53 个完整 `YYYY-MM-DD - 房间标题` 一一对应，可见 15 至 19 位 recordedAt 为 `0`；截图为 `/private/tmp/ugreen-serial-ui-acceptance-20260819-231921.jpeg`。
- 本次生产 RED 的直接根因不是媒体或 episode identity：NAS JS/Gzip 补丁正确，但 UGREEN WebView 仍执行旧 bundle；`video_serv` 未重新读取入口，同时 macOS UGREEN ClientProxy/Chromium HTTP Cache 保留旧响应。仅对 `video_serv` 执行官方 `systemctl reload`（HUP），PID `29889` 和启动时间均未改变；客户端缓存先锚定回滚再清除。未修改容器、UGREEN DB、媒体、NFO 或长 identity。NAS 回滚根为 `/volume2/docker/bililive-go/backups/ugreen-serial-label-20260819-222346/`，客户端缓存回滚根为 `~/Library/UGREEN_Nas_Pro/cache-backups/ugreen-serial-label-20260819-231158/`。
- 用户随后明确不接受 `MM-DD` 作为最终产品行为，序号标签子项已按新规格完成：未知长 episode 在完整已排序 `episodeList` 中取 1-based ordinal，底层 recordedAt identity、点击关联和完整日期 tooltip 不变。code review 先后发现并用 RED 锁定两项问题：date-v2 会越界启用三个卡片 seam，以及两个不同 terminal state 会被折叠成同一 `already-patched`；修复后 focused `10/10`、真实 canonical dry-run、`node --check`、Node `24.19.0` 的 `make build-web dev`、`make lint`、`make test` 和 `git diff --check` 均通过，最终 Standards/Spec 双轴复审均为 `0 findings`。
- 生产写前门禁为 active `0`、running `0`、update `idle`、ffmpeg `0`、近两分钟媒体写入 `0`；pending `4` 不消费影视中心前端资产。根因进一步收敛为 UGREEN ClientProxy 直接请求 canonical `app.2564f623.js`，而不是 `index.html` 已引用的 content-hash bundle。只对 canonical JS/Gzip 做一次 fail-closed `DATE -> ORDINAL` 原子替换；两份解压内容 SHA-256 均为 `4feb16590643c7a754c0ff94ede8370f98736bf472cb5797d3c1f44ff8993983`、date/hidden/ordinal 为 `0/0/1`，保持 `root:root 755`。原始资产位于 `/volume2/docker/bililive-go/backups/ugreen-serial-ordinal-canonical-20260819-235200/canonical-original/`；`video_serv` 仅 HUP，PID `29889` 与启动时间不变。
- UGREEN Chromium HTTP Cache 先原子备份为 `~/Library/Application Support/com.ugreen.desktop/Cache.codex-backup-20260819-2354` 后重开客户端。Computer Use 真页最终验收：`建筑师 linkai` 53 个标签严格连续为 `1..53`，`MM-DD` 标签 `0`、可见 15 至 19 位 recordedAt `0`、完整 `YYYY-MM-DD - 房间标题` tooltip `53`；首两项对应 `2026-05-15/05-17`，末两项对应 `2026-08-17/08-18`。截图为 `/private/tmp/ugreen-ordinal-ui-green-20260819-235653.jpeg`。
- `23:53 +08` 最小后验：Bililive app SHA 仍为 `44e87a4`、update `idle`、active recordings `0`、pipeline running/pending `0/4`，媒体、NFO、UGREEN DB、容器和长 identity 均未修改。仅序号标签子项完成；卡片整卡点击、首卡裁切和封面仍是独立未完成项。不得删除或重命名 MP4，也不得清理 `chronological-renumber-20260815-082317` 恢复备份。
- `2026-08-20 00:18 +08` 用户明确放弃本次 UI/cache 即时回退后完成精确清理：NAS 的 `ugreen-serial-label-20260819-222346`、`ugreen-serial-ordinal-20260819-233338`、`ugreen-serial-ordinal-canonical-20260819-235200` 三个回退根，macOS 的 `ugreen-serial-label-20260819-231158`、`ugreen-serial-ordinal-20260819-234152` 和 `Cache.codex-backup-20260819-2354` 三个缓存备份，以及未引用的 `serial-55cafd38` pair/临时 helper 均已删除；上述历史路径仅保留为审计记录，不再是有效回滚点。删除后活动/canonical 四份解压内容仍完全相同且 SHA-256 为 `4feb1659...`，index 唯一引用 ordinal bundle；`chronological-renumber-20260815-082317` 约 735GB 恢复备份明确未触及并继续保留。
