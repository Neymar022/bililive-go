...
- 烧录发布契约与积压恢复：按[已确认两批方案](implementation-status.md#2026-09-05-烧录发布契约根因修复方案澄清)实施；PR #51/#52 均已部署，运行 SHA/镜像/worker/1262 个既存媒体后验通过。09-06 用户批准停止全部房间并优先部署，已持久化停止、删除本次两份临时录制文件约 465 MB，未动历史输入。天津 60 集/熊小电 4 集 NFO 和四行 PG 已修复，播放关联/其他字段不变。历史采用因启动清日志导致原封口证据缺失而未执行；解析 500 仅随重启暂时恢复，仍需持久修复。当前不再受录制门禁阻塞，heartbeat 暂停，剩余源码/证据与真实客户端验收不得伪标完成。
- 单次/连续录制：PR #52 `80ab0ec` 已发布并用原 Compose/latest 最小 app 部署；既有 full gate、focused/race、正式双轴审查与本地 E2E 12 项通过。10:21 生产 API 字段、旧 continuous 兼容、真实 NAS Web 添加窗口默认 once/切换连续均通过；所有监听保持停止。真实单次结束/重启和投影仪验收未完成，不以历史积压全部恢复为前置。
