...
- 烧录发布契约与积压恢复：按[已确认两批方案](implementation-status.md#2026-09-05-烧录发布契约根因修复方案澄清)实施；PR #51 已合并 master 且镜像发布成功，生产部署、定向历史采用/元数据修复/安全续跑及真实客户端后验仍待完成。
- 单次/连续录制：配置、SQLite 额度、最小解码、运行入口/API/UI 已接通，本地 full gate、focused/race、正式双轴审查与 Chrome E2E 12 项通过；独立 PR/CI/部署和真实验收待完成，不以历史积压全部恢复为前置。
