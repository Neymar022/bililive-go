...
class SegmenterTest(unittest.TestCase):
    ...
    def test_merges_adjacent_short_segments_when_combined_chunk_fits_budget(self):
        segments = split_segments_for_timeline(
            [
                {"index": 1, "start_ms": 0, "end_ms": 900, "text": "告诉"},
                {"index": 2, "start_ms": 900, "end_ms": 1800, "text": "我一声"},
            ],
            max_chars=8,
        )

        self.assertEqual(["告诉我一声"], [segment["text"] for segment in segments])
        self.assertEqual(0, segments[0]["start_ms"])
        self.assertEqual(1800, segments[0]["end_ms"])
    ...
    def test_repairs_common_asr_numeric_misrecognitions_in_warning_phrases(self):
        segments = split_segments_for_timeline(
            [
                {
                    "index": 1,
    ...
    def test_applies_best_practice_timing_envelope_without_overlap(self):
        segments = split_segments_for_timeline(
            [
                {"index": 1, "start_ms": 1000, "end_ms": 1600, "text": "第一句"},
                {"index": 2, "start_ms": 3000, "end_ms": 3600, "text": "第二句"},
            ],
            max_chars=8,
        )

        self.assertEqual(1000, segments[0]["start_ms"])
        self.assertEqual(2500, segments[0]["end_ms"])
        self.assertEqual(2500, segments[1]["start_ms"])
        self.assertEqual(3800, segments[1]["end_ms"])
        self.assertTrue(segments[0]["end_ms"] <= segments[1]["start_ms"])
