...
# endpoint/token 可用 BILINOTE_KNOWLEDGE_INGEST_URL 和 BILINOTE_INGEST_TOKEN 环境变量覆盖`, "")
		...
		if knowledgeSyncNode != nil {
			...
			setFieldComment(knowledgeSyncNode, "link", "# 可选：覆盖是否生成原片跳转；留空时使用 Bililive-go 默认值", "")
			setFieldComment(knowledgeSyncNode, "screenshot", "# 可选：覆盖是否生成原片截图；留空时使用 Bililive-go 默认值", "")
