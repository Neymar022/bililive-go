...
func rerunSubtitleRecord(writer http.ResponseWriter, r *http.Request) {
	...
	if sourcePath == "" || !fileExistsOnDisk(sourcePath) {
		if resolvedSourcePath, resolveErr := subtitle.ResolveSourcePath(videoPath, sourceRoot); resolveErr == nil {
			sourcePath = resolvedSourcePath
		}
	}
	processingPath := selectSubtitleRerunInputPath(videoPath, sourcePath)
	...
	subtitle.InvalidateRecordCache()

	writeJSON(writer, commonResp{Data: map[string]any{"task_id": task.ID}})
}

func selectSubtitleRerunInputPath(videoPath, sourcePath string) string {
	if fileExistsOnDisk(sourcePath) {
		return sourcePath
	}
	if fileExistsOnDisk(videoPath) {
		return videoPath
	}
	return sourcePath
}
...
func putSubtitleSettings(writer http.ResponseWriter, r *http.Request) {
	...
	// 配置变更可能改了 source_root / library_root / retention_days，
	// 缓存里旧的 record 列表已与新 roots 无关，必须重建。
	subtitle.InvalidateRecordCache()
	writeJSON(writer, commonResp{Data: "OK"})
}
...
func getSubtitleRoots() (sourceRoot string, libraryRoot string, retentionDays int) {
	...
	return absolutizePath(sourceRoot), absolutizePath(libraryRoot), cfg.Subtitle.RetentionDays
}
