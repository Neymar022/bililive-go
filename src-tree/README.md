...
云端默认使用阿里百炼 `qwen3-asr-flash-filetrans`，本地可选 `faster-whisper`。

详细配置和部署说明见 [字幕增强文档](docs/subtitle.md)。
...
docker compose up
```
此时默认使用 `config.docker.yml` 文件作为程序的配置文件。

如果启用了字幕增强，推荐使用双目录结构：

- `Videos/source/`：原始录屏目录
- `Videos/library/`：正式媒体库目录
