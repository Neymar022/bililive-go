...
def _apply_alpha_scale(color: tuple[int, int, int, int], scale: float) -> tuple[int, int, int, int]:
    red, green, blue, alpha = color
    next_alpha = max(0, min(255, round(alpha * scale)))
    return red, green, blue, next_alpha
...
def render_cue_png(
    ...
    *,
    ...
    layout = layout_vizard_classic_cn(text, video_width=video_width, video_height=video_height)
    ...
    border_width = float(style_vars.get("card-border-width", "2"))
    if border_width > 0:
        outline_width = 1 if border_width < 1 else max(1, round(border_width))
        outline_color = card_stroke if border_width >= 1 else _apply_alpha_scale(card_stroke, border_width)
        draw.rounded_rectangle(
            (left, top, right, bottom),
            radius=card_radius,
